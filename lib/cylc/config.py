...
class SuiteConfig(object):
    ...
    def __init__(self, suite, fpath, template_vars=None,
        ...
        for mod_name_as, mod_name in self.pcfg.modules.items():
            ...
            try:
                t_vars = self.cfg['modules'][mod_name_as]
            except KeyError:
                t_vars = {}
            pcfg = RawSuiteConfig.get_inst(
    ...
    def _load_module(self, mod_name, mod_cfg):
        # TODO incorporate other needed module config and reject the rest.
        deps = self.cfg['scheduling']['dependencies']
        # Incorporate suite parameters.
        try:
            params = mod_cfg['cylc']['parameters']
        except KeyError:
            pass
        else:
            if 'cylc' not in self.cfg:
                self.cfg['cylc'] = {}
            if 'parameters' in self.cfg['cylc']:
                self.cfg['cylc']['parameters'].update(
                    mod_cfg['cylc']['parameters'])
            else:
                self.cfg['cylc']['parameters'] = (
                    mod_cfg['cylc']['parameters'])
        ...
        for namespace, cfg in mod_cfg['runtime'].items():
            ...
            if new_namespace in self.cfg['runtime']:
                self.cfg['runtime'][new_namespace].update(cfg)
            else:
