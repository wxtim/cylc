...
\section{Cycling, Cycle Points, and Workflows}
...
\end{myitemize}
This section defines the terminology associated with cycling and describes
how to make a workflow that cycles a model and other associated jobs.

\subsection{Cycle Points}

In cylc terminology, a {\em cycle point} is a particular date-time (or integer)
point in a sequence. If a task has an hourly date-time cycle that starts at
2014-01-01T00 and ends at 2014-01-02T00, the first cycle point will be
...
tasks with the current cycle point have finished (or even when a particular
wall clock time is reached in real time operation). As explained in the
...
\subsection{Workflows For Cycling Tasks}
\label{Workflows For Cycling Tasks}

% TODO - replace all references to "parameterized cycling" and "dynamic cycling".

\subsubsection{Static Workflows For Cycling Tasks}

It is possible to cycle a model with a workflow engine that does not even
understand cycling, if each successive model job is represented by a {\em
different task} in the workflow. You can do this in cylc too by using parameter
expansion (~\ref{Parameterized Cycling}) or Jinja2 loops (\ref{Jinja2}) to
generate tasks at start up so that (for instance) task \lstinline=model-r1=
executes \lstinline=model.exe= for the first cycle point, task
\lstinline=model-r2= for the second point, and so on. The parameter values used
in the task names can be used along with a date-time interval to compute the
cycle point for each model run.

A static workflow like this is necessarily finite: every task from start to
finish must be defined at start-up, and the suite engine has to manage the
state of them all for the duration of the run. This could have consequences for
performance and monitoring of very large suites.

\subsubsection{Cycling Workflows For Cycling Tasks}

The best way to cycle a model with cylc is usually a {\em cycling workflow} in
which each successive model run is executed by {\em a new instance of the same
task}.  For example, task \lstinline=model= might execute every
\lstinline=model.exe= job, but a new cycle point will be associated with the task
each time, and passed to the task job for use as a model start time or
whatever.  In effect, the suite defines a pattern for generating more workflow
at run time as needed.  Cylc does this with no hard boundaries between cycle
points, and the scheduling efficiency (unless deliberately restricted) is the
same as for a static workflow.  Unlike the static case though the scheduler
does not need to be aware of the entire workflow at once, and it can continue
cycling indefinitely, so this is really the only way to run large ongoing
operational systems.

Most of the User Guide is concerned with this kind of workflow, with cycling
defined by ISO 8601 standard based date-time {\em recurrence expressions}.
Multiple cycling sequences can be used in one suite. See
Section~\ref{ConfiguringScheduling}.

\subsubsection{Mixed Cycling}

For completeness, the two cycling approaches can be combined within a single
suite. In a daily cycling workflow, for instance, long daily model runs could
be split into multiple shorter runs by parameterized static cycling. However,
it is usually simpler to just use a cycling workflow with a shorter interval.
...
Section~\ref{Workflows For Cycling Tasks}. In iterative cycling (which is
recommended in most cases) each instance of a cycled job is run by the same
logical task at
