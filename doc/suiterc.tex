...
\label{RunaheadLimitRef}

The runahead limit determines how far the fastest tasks in a suite
can get ahead of the slowest ones. Tasks exceeding the runahead 
limit go into a special held state until others have caught up
sufficiently. The default runahead limit is twice the smallest cycling
interval specified in the dependency graph. See also 
Section~\ref{RunaheadLimit}.

\begin{myitemize}
    \item {\em type:} an ISO8601 duration (e.g. `P1D') or an integer
        duration for integer cycling.
    \item {\em default:} (twice the smallest cycling interval)
\end{myitemize}
