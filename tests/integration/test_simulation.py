...
from cylc.flow.simulation import internal_task_status_check
...
async def internal_task_status_check_setup(
...
def test_false_if_not_running(internal_task_status_check_setup, monkeypatch):
    schd, itasks, msg_q = internal_task_status_check_setup

    # False if task status not running:
    assert internal_task_status_check(msg_q, itasks) is False


def test_internal_task_status_check_sets_started_time(
    internal_task_status_check_setup
):
    """But internal_task_status_check still returns False"""
    schd, _, msg_q = internal_task_status_check_setup
    ...
    assert internal_task_status_check(msg_q, [one_1066]) is False
    assert one_1066.summary['started_time'] is not None


def test_task_finishes(internal_task_status_check_setup, monkeypatch):
    """...and an appropriate message sent.

    Checks all possible outcomes in internal_task_status_check where elapsed time is
    ...
    assert internal_task_status_check(msg_q, [fail_all_1066]) is False

    # After simulation time is up:
    monkeypatch.setattr('cylc.flow.simulation.time', lambda: 12)
    assert internal_task_status_check(msg_q, [fail_all_1066]) is True
...
def test_task_sped_up(internal_task_status_check_setup, monkeypatch):
    ...
    monkeypatch.setattr('cylc.flow.simulation.time', lambda: 0)
    assert internal_task_status_check(msg_q, [fast_forward_1066]) is False
    monkeypatch.setattr('cylc.flow.simulation.time', lambda: 29)
    assert internal_task_status_check(msg_q, [fast_forward_1066]) is False
    monkeypatch.setattr('cylc.flow.simulation.time', lambda: 31)
    assert internal_task_status_check(msg_q, [fast_forward_1066]) is True
